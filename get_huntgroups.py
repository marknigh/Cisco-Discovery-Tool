...
class GetHuntGroups():
    ...
    def get_list_pilot(self):
        ...
        try:
            list_hunt_pilots = self._service.listHuntPilot( searchCriteria = { 'pattern': '%' }, returnedTags = {} )
    ...
    def get_pilot_details(self, uuid):
        try:
            pilot_detail = self._service.getHuntPilot( uuid = uuid )
            return pilot_detail
    ...
    def get_hunt_list(self, uuid):
        try:
            hunt_list = self._service.getHuntList( uuid = uuid )
            return hunt_list
    ...
    def get_hunt_members(self, uuid):
        try:
            line_group = self._service.getLineGroup( uuid = uuid )
            return line_group
