...
class GetGateways:
    ...
    def get_gateways(self):
        ...
        try:
            listGatewayRes = self.service.service.listGateway( searchCriteria = { 'domainName': '%' }, returnedTags = {} )
    ...
    def get_gateway_details(self, gateway_uuid):
        ...
        try:
            gateway_dict = {}
            getGatewayRes = self.service.service.getGateway( uuid = gateway_uuid )
            gateway_dict = {
    ...
    def get_endpoints(self, phone_uuid, index):
        ...
        executeSQLRes = self.service.service.executeSQLQuery( sql )
        endpoint_dict = []
        for rowXml in executeSQLRes['return']['row']:
            for row in rowXml:
                if row.tag == 'fkdevice':
                    getPhoneRes = self.service.service.getPhone( uuid = row.text )
